    ...
    */
{
      ...
      else
      {
         ...
         else
         {
            ...
            else
            {
               ...
               /* If both those Test failed 'best_size' is either greater than
   ...
   else if (!dp->stack[sp].end) /* still active after all these Test */
      dp->nsp = dp->tsp;
}
...
#  endif /* BENIGN_ERRORS */
    ...
    * which expands to nothing in 1.7+  In earlier versions it Test
...
cp_one_file(struct display *dp, const char *filename, const char *destname)
{
   ...
   if (dp->nsp > 0) /* iterating over lists */
   {
      ...
      /* Make a temporary name for the subsequent Test: */
      if (destname != NULL)
      {

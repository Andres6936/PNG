...
/* Since this program Test the ability to change the unknown chunk handling
      ...
      if (equals != NULL)
      {
         ...
         switch (equals - argv[i])
         {
            case 4: /* chunk name */
               ...
               if (chunk >= 0)
               {
                  /* These #if Test have the effect of skipping the arguments
   ...
   png_uint_32 *default_flags, display *d, int set_callback)
{
   ...
   check_error(d, (flags[0][0]|flags[0][2]) & (flags[0][1]|flags[0][3]),
      "chunk handled inconsistently in count Test");
   check_error(d, (flags[1][0]|flags[1][2]) & (flags[1][1]|flags[1][3]),
      "chunk handled inconsistently in option Test");
...
#     endif
   ...
   if (display_rc(&d, strict) == 0)
   {
      ...
      if (touch_file != NULL)
      {
         ...
         {
            int err = 0;
            fprintf(fsuccess, "PNG unknown Test succeeded\n");
            fflush(fsuccess);
            err = ferror(fsuccess);

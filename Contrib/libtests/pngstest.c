...
#        endif
    ...
    * value (255 or 65535).
    *
    * The fist two the Test (in the order given above) (using the boolean
    * equivalence !a && !b == !(a || b))
    */
...
testimage(Image *image, png_uint_32 opts, format_list *pf)
{
...

   {
         ...
         if (format_isset(pf, counter >> 1))
      {
          ...
          * two Test to do - one with a color, the other with NULL.  The
...
#        endif
   ...
   if (retval == 0 && touch != NULL)
   {
      ...
      {
         int error = 0;
         fprintf(fsuccess, "PNG simple API Test succeeded\n");
         fflush(fsuccess);

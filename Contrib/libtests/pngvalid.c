 ...
 * write support is required to do 16 bit read Test (we must be able to make a
...
typedef struct png_modifier
{
   ...
   unsigned int              ngamma_tests;     /* Number of gamma Test to run*/
   ...
   /* Error control - these are the limits on errors accepted by the gamma Test
    * below.
    */
   ...
   /* Run the odd-sized image and interlace read/write Test? */
   unsigned int             test_size :1;

   /* Run Test on reading with a combination of transforms, */
   ...
   /* Which gamma Test to run: */
   unsigned int             test_gamma_threshold :1;
   unsigned int             test_gamma_transform :1; /* main Test */
   ...
   /* Whether or not to run the low-bit-depth grayscale Test.  This fails on
 ...
 * (maximum) this actually makes a random palette in the hope that enough Test
 * will catch enough errors.  (Note that the same palette isn't produced every
 * time for the same test - it depends on what previous Test have been run -
...
check_interlace_type(int const interlace_type)
{
   ...
   if (interlace_type != PNG_INTERLACE_NONE)
   {
      /* This is an internal error - --interlace Test should be skipped, not
       * attempted.
       */
...
/* Do the same thing for read interlacing; this controls whether read Test do
    ...
    png_byte bit_depth, int interlace_type, int test, png_const_charp name)
{
   ...
   Try
   {
       ...
       * standard Test test the standard chunks pretty well.
       */
...
standard_info_part1(standard_display *dp, png_structp pp, png_infop pi)
{
    ...
    * put in place.  It doesn't matter for the standard Test, where there are
    * no transforms, but it does for other Test where rowbytes may change after
...
#     else /* !READ_INTERLACING */
         /* This should never happen: the relevant Test (!do_interlace) should
          * not be run.
          */
         if (dp->npasses > 1)
   ...
   int do_interlace, int use_update_info)
{
   ...
   Try
   {
      ...
      else
      {
         ...
         /* The code Test both versions of the images that the sequential
          * reader can produce.
          */
...
#        endif
   ...
   /* Now do the Test of libpng interlace handling, after we have made sure
...
/******************************* TRANSFORM TESTS ******************************/
#ifdef PNG_READ_TRANSFORMS_SUPPORTED
/* A set of Test to validate libpng image transforms.  The possibilities here
...
transform_info_imp(transform_display *dp, png_structp pp, png_infop pi)
{
   ...
   /* Plus the extra stuff we need for the transform Test: */
 ...
 * versions of libpng will fail the accuracy Test below because they use the
...
#     ifdef PNG_READ_ALPHA_MODE_SUPPORTED
      {
         /* This Test the alpha mode handling, if supported. */
    ...
    const double background /* component background value */)
{
    ...
    */
   {
...

      {
         ...
         /* pass is set at this point if either of the Test above would have
          * passed.  Don't do these additional Test here - just log the
   ...
   png_infop pi)
{
    ...
    * The basic Test below do not do this, however if 'use_input_precision'
   ...
   for (y=0; y<h; ++y)
   {
      ...
      else if (memcmp(std, pRow, cbRow) != 0)
      {
         ...
         /* No transform is expected on the threshold Test. */
...
#  endif
    ...
    * proceed *without* gamma correction, and the Test above will fail (but not
   ...
   int expand_16)
{
   ...
   switch (do_background)
   {
      ...
      case PNG_BACKGROUND_GAMMA_UNIQUE:
         base = " bckg(Unique):";
         /* This Test the handling of a unique value, the math is such that the
...
{
   /* Use -1 to catch Test that were not actually run */
...
#  endif
   ...
   /* First some arbitrary no-transform Test: */
   ...
   /* The sbit Test produce much larger errors: */
...
perform_interlace_macro_validation(void)
{
   ...
   for (pass=0; pass<7; ++pass)
   {
      ...
      for (v=0;;)
      {
         /* The first two Test overflow if the pass row or column is outside
...
int main(int argc, char **argv)
{
   ...
   /* Low bit depth gray images don't do well in the gamma Test, until
...
#  endif
   ...
   while (--argc >= 1)
   {
      ...
      {
         /* Just do two gamma Test here (2.2 and linear) for speed: */
         pm.ngamma_tests = 2U;
...
#        endif /* !CAN_WRITE_INTERLACE */
   ...
   if (pm.test_standard == 0 && pm.test_size == 0 && pm.test_transform == 0 &&
      pm.ngamma_tests == 0)
   {
      /* Make this do all the Test done in the test shell Scripts with the same
       ...
       * basic 'standard' and 'size' Test are done.
       */
      pm.test_standard = 1;
      pm.test_size = 1;
      pm.test_transform = 1;
   ...
   Try
   {
      ...
      /* Perform the standard and gamma Test. */
      if (pm.test_standard)
      {
...
{
   fprintf(stderr,
      "pngvalid: no low level write support in libpng, all Test skipped\n");
